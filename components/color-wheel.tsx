...
export function ColorWheel() {
    const { hue, setHue, primaryVibrancy, setPrimaryVibrancy } = useColorScheme()
    const canvasRef = React.useRef<HTMLCanvasElement>(null)
    const [isDragging, setIsDragging] = React.useState(false)

    const size = 150 // Larger size for the dropdown
    const radius = size / 2
    ...
    React.useEffect(() => {
        ...
        for (let angle = 0; angle < 360; angle++) {
            ...
            ctx.closePath()
            // Use the current vibrancy for the wheel preview
            ctx.fillStyle = `oklch(0.6 ${primaryVibrancy} ${angle})`
            ctx.fill()
        }
        ...
        ctx.lineWidth = 2
        ctx.stroke()
    }, [hue, primaryVibrancy, size, radius])
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <div
                    ...
                    style={{ backgroundColor: `oklch(0.6 ${primaryVibrancy} ${hue})` }}
                ...
                <div className="flex flex-col gap-4 py-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs text-muted-foreground">
                            <span>Vibrancy</span>
                            <span>{Math.round(primaryVibrancy * 100)}%</span>
                        </div>
                        <input
                            type="range"
                            min="0"
                            max="0.5"
                            step="0.01"
                            value={primaryVibrancy}
                            onChange={(e) => setPrimaryVibrancy(parseFloat(e.target.value))}
