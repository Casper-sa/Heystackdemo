...
export function BackgroundColorWheel() {
    const { backgroundHue, setBackgroundHue, backgroundVibrancy, setBackgroundVibrancy } = useColorScheme()
    ...
    React.useEffect(() => {
        ...
        for (let angle = 0; angle < 360; angle++) {
            ...
            ctx.fillStyle = `oklch(0.98 ${backgroundVibrancy} ${angle})`
            ctx.fill()
        }

        // Draw indicator at current hue position
        const indicatorAngle = (backgroundHue - 90) * (Math.PI / 180)
        ...
        ctx.lineWidth = 2
        ctx.stroke()
    }, [backgroundHue, backgroundVibrancy, size, radius])
    ...
    const handleInteraction = React.useCallback(
        (clientX: number, clientY: number) => {
            ...
            setBackgroundHue(Math.round(angle))
        },
        [radius, setBackgroundHue]
    )
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <div
                    ...
                    style={{ backgroundColor: `oklch(0.98 ${backgroundVibrancy} ${backgroundHue})` }}
                ...
                <div className="flex flex-col gap-4 py-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs text-muted-foreground">
                            <span>Vibrancy</span>
                            <span>{Math.round(backgroundVibrancy * 100)}%</span>
                        </div>
                        <input
                            type="range"
                            min="0"
                            ...
                            onChange={(e) => setBackgroundVibrancy(parseFloat(e.target.value))}
