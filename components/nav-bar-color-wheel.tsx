...
export function NavBarColorWheel() {
    const { navBarHue, setNavBarHue, navBarVibrancy, setNavBarVibrancy } = useColorScheme()
    ...
    React.useEffect(() => {
        ...
        for (let angle = 0; angle < 360; angle++) {
            ...
            ctx.fillStyle = `oklch(1 ${navBarVibrancy} ${angle})`
            ctx.fill()
        }

        // Draw indicator at current hue position
        const indicatorAngle = (navBarHue - 90) * (Math.PI / 180)
        ...
        ctx.lineWidth = 2
        ctx.stroke()
    }, [navBarHue, navBarVibrancy, size, radius])
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <div
                    ...
                    style={{ backgroundColor: `oklch(1 ${navBarVibrancy} ${navBarHue})` }}
                    title="Nav Bar Color Settings"
                />
            </DropdownMenuTrigger>
            <DropdownMenuContent className="p-4 w-64" align="center">
                <DropdownMenuLabel>Nav Bar Color</DropdownMenuLabel>
                <DropdownMenuSeparator />
                ...
                <div className="flex flex-col gap-4 py-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs text-muted-foreground">
                            <span>Vibrancy</span>
                            <span>{Math.round(navBarVibrancy * 100)}%</span>
                        </div>
                        <input
                            type="range"
                            min="0"
                            max="0.5"
                            step="0.005"
                            value={navBarVibrancy}
                            onChange={(e) => setNavBarVibrancy(parseFloat(e.target.value))}
