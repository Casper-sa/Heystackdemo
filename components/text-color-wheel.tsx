...
export function TextColorWheel() {
    const { textHue, setTextHue, textVibrancy, setTextVibrancy } = useColorScheme()
    ...
    React.useEffect(() => {
        ...
        for (let angle = 0; angle < 360; angle++) {
            ...
            ctx.closePath()
            // Use current vibrancy for preview (light mode text color - dark for readability)
            ctx.fillStyle = `oklch(0.145 ${textVibrancy} ${angle})`
            ctx.fill()
        }

        // Draw indicator at current hue position
        const indicatorAngle = (textHue - 90) * (Math.PI / 180)
        ...
        ctx.lineWidth = 2
        ctx.stroke()
    }, [textHue, textVibrancy, size, radius])
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <div
                    ...
                    style={{ backgroundColor: `oklch(0.145 ${textVibrancy} ${textHue})` }}
                ...
                <div className="flex flex-col gap-4 py-2">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between text-xs text-muted-foreground">
                            <span>Vibrancy</span>
                            <span>{Math.round(textVibrancy * 100)}%</span>
                        </div>
                        <input
                            type="range"
                            min="0"
                            ...
                            step="0.005"
                            value={textVibrancy}
                            onChange={(e) => setTextVibrancy(parseFloat(e.target.value))}
